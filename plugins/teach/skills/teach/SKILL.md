...
4. Write the lesson data as JSON. Set `meta.agent` to `codex` when running in Codex and `claude` when running in Claude Code so copied follow-up questions use the correct command. Default to `~/teach-lessons/<lesson-slug>/lesson.json` so generated artifacts do not dirty the product repository. Reuse that file for follow-up revisions.
